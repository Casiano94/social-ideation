    ...
    if _is_story_from_admin(sn_app, post):
        ...
        if len(hashtags) > 0 and initiative:
            ...
            if campaign:
              ...
              """ le paso el comentario sin hashtag y retorna el hashtag correspondiente al post """
             hashting =  assignment_hashtag(post['text'])  
             out_hash = string.replace(hashting,"#","")
             """ aqui realizo la consulata a la bd para obtener los comentarios con ese hashtag """
            ideas_hashtea = Idea.objects.filter(campaign_hashtag = out_hash)
            learn(out_hash,ideas_hashtea)
            _send_notification_comment(sn_app, post, initiative, 'Hi \'{}\' , we didn\'t found any hashtag, according to your comment they are'+hashting)
